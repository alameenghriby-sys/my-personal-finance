...
# تحليل النص (النسخة الكاملة - مصارف + ديون + كاش) 🧠💰
def analyze_text(text):
    prompt = f"""
    أنت محاسب شخصي ليبي ذكي. حلل النص: '{text}'
    
    🚨 قواعد الديون (الأهمية القصوى):
    1. **دين ليا (أصول):** إذا احتوى النص على "دين ليا"، "سلفته"، "عطيته"، "عند فلان"، "يبي يرجعلي" -> النوع type = 'lend'.
    2. **دين عليا (التزامات):** إذا احتوى النص على "دين عليا"، "تسليفت"، "خديت سلفة"، "يبي مني" -> النوع type = 'borrow'.
    3. **سداد لي:** "ردلي"، "خلصني" -> type = 'repay_in'.
    4. **سداد مني:** "رديت"، "خلصته" -> type = 'repay_out'.

    🏦 قواعد المصارف:
    1. **Wahda:** "موبي كاش"، "إي دي"، "ED"، "المدار"، "الوحدة".
    2. **NAB:** "ناب"، "شمال أفريقيا".
    3. **Cash:** إذا لم يذكر أي بنك، الحساب هو Cash.

    ⚙️ القواعد العامة:
    - "شراء"، "صرف"، "خصم" -> type = 'expense'.
    - "إيداع"، "إضافة"، "راتب" -> type = 'income'.
    - amount رقم فقط.

    المطلوب JSON: amount, item, category, type, account.
    """
    ...
    try:
        ...
        # --- معالجة الحسابات الافتراضية ---
        if not data.get('account'):
            data['account'] = "Cash"
            
        # تنظيف البيانات
        if not data.get('item') or str(data['item']).strip() == "":
            data['item'] = "عملية دين" if data.get('type') in ['lend', 'borrow'] else "عملية عامة"
            
        # تصنيف الديون تلقائياً
        if data.get('type') in ['lend', 'borrow', 'repay_in', 'repay_out']:
            data['category'] = 'ديون'
