...
def analyze_text(text):
    prompt = f"""
    أنت محاسب شخصي للمهندس الأمين.
    حلل النص: '{text}'
    
    القواعد الصارمة جداً (طبقها بالترتيب):
    1. إذا احتوى النص على كلمة "اضافة" أو "إيداع" أو كان مجرد رقم (مثال: "500" أو "اضافة 500") -> النوع type = "income"، والتصنيف category = "دخل".
    2. الحساب (account): إذا لم يذكر المستخدم اسم "الوحدة" أو "Wahda" أو "شمال أفريقيا" أو "NAB" صراحة -> الحساب فوراً = "Cash".
    3. إذا لم يوجد وصف للعملية، اجعل item = "إيداع نقدي".
    4. استخرج JSON: {{ "amount": number, "item": string, "category": string, "type": string, "account": string }}
    """
    try:
        ...
        # --- تصحيحات إجبارية (Python Logic) ---
        # 1. لو الموديل نسى يحدد الحساب، نخليه كاش "غصباً عنه"
        if not data.get('account') or data['account'] not in ['Wahda', 'NAB']:
            data['account'] = 'Cash'
            
        # 2. لو البيان فاضي والعملية دخل، نسميها إيداع
        if (not data.get('item') or data['item'] == "عملية عامة") and data.get('type') == 'income':
            data['item'] = "إيداع نقدي"

        # 3. توحيد التصنيف للدخل
        if data.get('type') == 'income' and data.get('category') == 'عام':
            data['category'] = 'دخل'
